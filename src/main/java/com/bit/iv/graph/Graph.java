...
import com.bit.iv.algorithms.find.path.PathAlgorithm;
...
public class Graph {
    ...
    public AlgorithmPerformance findPath(PathAlgorithm pathAlgorithm) {
    ...
    public long displayPath(BufferedImage bufferedImage) {
        ...
        while (vertex.getParent() != null) {
            ...
            if(from_x == to_x) {
                if(from_y < to_y) {
                    while (from_y <= to_y) {
                        bufferedImage.setRGB(from_x, from_y, ImageProcessor.pathColor.getRGB());
                        from_y++;
                    }
                } else {
                    while (from_y >= to_y) {
